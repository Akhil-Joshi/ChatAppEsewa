...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        # 🚫 Prevent crash from invalid/empty input
        ...
        if file_data:
            try:
                ...
                await self.send(text_data=json.dumps({
                    "type": "error",
                    "message": f"File decoding error: {str(e)}"
                }))
                return
        ...
        saved_message = await self.save_message(message, file_obj)
    ...
    def save_message(self, message, file_obj=None):
        try:
